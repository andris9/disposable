...
function loadMailbox(mailbox, callback){
	dbObj.collection(config.mongo.collection, function(err, collection){
        ...
        collection.find({mailbox: mailbox},
          {from: true, mailbox: true, "message.subject": true, read:true, received: true, rawsize: true, "message.from": true, "message.to": true, "message.cc": true},
                    ...
                    if(elm.message.attachments){
                        for(var i=0, len = elm.message.attachments.length; i<len; i++){
                            if(elm.message.attachments[i].contentId == cid){
                                return prefix + config.mailbox.urlprefix + "/attachment/"+returnElm.id+"/"+i+"/" + elm.message.attachments[i].generatedFileName.replace(/</g,"&lt;").replace(/>/g,"&gt;");
                            }
                        }
                    }    
